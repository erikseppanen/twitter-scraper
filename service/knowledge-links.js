(() => {
  ...
  style.textContent += '.related-open .tweet-list{width:50%;margin-left:auto;margin-right:50%}.related-panel{position:fixed;right:16px;top:170px;bottom:16px;width:calc(50% - 32px);padding:0;overflow:auto;overscroll-behavior:contain;background:var(--bg-card,#fff);border:1px solid var(--border-color,#ddd);border-radius:16px}.related-panel>nav,.related-panel>h2,.related-panel>p{margin:12px}.related-active{outline:2px solid var(--link-color,#1d9bf0)}@media(max-width:760px){.related-open .tweet-list{width:auto;margin:0 auto}.related-panel{position:static;width:auto;margin:16px 0}}';
  ...
  let sequence = 0, anchor, trail = [];
  function follow(event, id) { if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.preventDefault(); open(id); }
  ...
  function card(tweet, selected = false) {
    ...
    if (!selected) { const related = el('a', 'Related →'); related.href = linkFor(tweet.tweetId); related.onclick = e => follow(e, tweet.tweetId); footer.append(related); }
    arrange(t); return c;
  }
  async function open(id, back = false) {
    const seq = ++sequence;
    if (!back) trail.push(id);
    if (anchor) anchor.after(panel); else if (!panel.isConnected) (document.querySelector('main') || document.body).prepend(panel);
    panel.hidden = false; document.body.classList.add('related-open');
    document.querySelector('.related-active')?.classList.remove('related-active');
    document.getElementById('tweet-' + id)?.classList.add('related-active');
    history.replaceState(null, '', '/archive/?related=' + encodeURIComponent(id));
    const tools = el('nav', null, 'related-tools');
    const prev = button('← Back', () => { trail.pop(); open(trail.at(-1), true); }); prev.disabled = trail.length < 2;
    tools.append(prev, button('Close', () => { ++sequence; panel.hidden = true; document.body.classList.remove('related-open'); document.querySelector('.related-active')?.classList.remove('related-active'); history.replaceState(null, '', '/archive/'); }));
    ...
    try {
      ...
      notice.textContent = 'Related to @' + (data.nodes[0].tweet.user?.screenName || 'unknown');
  ...
  function enhance() {
    for (const c of document.querySelectorAll('article[id^="tweet-"]')) {
      ...
      a.onclick = e => { if (!(e.metaKey || e.ctrlKey || e.shiftKey || e.altKey)) { anchor = c; trail = []; } follow(e, id); };
      const permalink = el('a', 'Permalink', 'tweet-permalink'); permalink.href = linkFor(id); permalink.title = 'Permanent link to this tweet';
      (c.querySelector('footer') || c).append(permalink);
      (c.querySelector('footer') || c).append(a);
      arrange(c);
    }
  }
  enhance(); new MutationObserver(enhance).observe(document.body, { childList:true, subtree:true });
  const initial = new URLSearchParams(location.search).get('related');
  if (/^\d+$/.test(initial || '')) open(initial);
})();

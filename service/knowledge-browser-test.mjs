...
try {
  ...
  assert.equal(await page.locator('.related-card[data-tweet-id="1"]').count(), 0);
  assert.equal(await page.locator('article#tweet-1').count(), 1);
  await page.getByRole('link', { name: 'Related →', exact: true }).first().click();
  await page.waitForFunction(() => document.querySelector('.related-panel')?.dataset.centerId !== '1');
  await page.getByRole('button', { name: '← Back', exact: true }).click();
  await page.waitForFunction(() => document.querySelector('.related-panel')?.dataset.centerId === '1');
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
  await page.goto(origin + '/tweet/1');
  await page.waitForURL('**/archive/?related=1');
  await page.locator('.related-card').first().waitFor();
  assert.equal(await page.locator('.related-card[data-tweet-id="1"]').count(), 0);
